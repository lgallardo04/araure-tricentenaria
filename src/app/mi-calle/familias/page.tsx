...
import {
  FiSearch, FiX, FiUsers, FiUser, FiChevronDown, FiChevronUp,
  FiHome, FiDownload, FiEdit2, FiSlash, FiTrash2, FiCheckCircle,
} from 'react-icons/fi';
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 px-4 text-slate-400 glass-card mx-auto mt-8">
          ...
          <p className="max-w-md mx-auto mb-6">Aún no has registrado ninguna familia. Dirígete a &quot;Censar&quot; para comenzar.</p>
        </div>
      ) : (
              ...
              <div key={fam.id} className={`glass-card overflow-hidden ${!viviendaActiva ? 'opacity-70 border-amber-500/20' : ''}`}>
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    {!viviendaActiva && (
                      <span className="ml-1 px-2 py-0.5 bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs rounded-full flex-shrink-0">
                        ⚠️ Vivienda Inactiva
                      </span>
                    )}
                  </div>
                ...
                {/* Expanded content */}
                {expandedId === fam.id && (
                  <div className="border-t border-slate-700/50 p-4 bg-slate-900/30 animate-fade-in space-y-4">
                      ...
                      {jefe && (
                        <div className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-sm border ${
                          ...
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <div className="min-w-0">
                              <span className={`truncate font-medium ${jefe.activo === false ? 'text-slate-500 line-through' : 'text-slate-200'}`}>
                                {jefe.nombre}
                              </span>
                              {jefe.activo === false && (
                          ...
                          {miembros.map((m) => (
                            <div key={m.id} className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-sm border ${
                              ...
                              <div className="flex items-center gap-2 min-w-0">
                                <FiUser className="w-3 h-3 text-slate-500 flex-shrink-0" />
                                <div className="min-w-0">
                                  <span className={`truncate ${m.activo === false ? 'text-slate-500 line-through' : 'text-slate-300'}`}>
                                    {m.nombre}
                                  </span>
                                  {m.parentesco && (
                                    <span className="text-slate-600 text-xs ml-1">({m.parentesco})</span>
                                  )}
                                  {m.activo === false && (
                                    <span className="ml-2 px-1.5 py-0.5 bg-red-500/10 border border-red-500/30 text-red-400 text-xs rounded-full">
